...
forecasters = ["C:\\Users\\uygar\\Desktop\\DataAnalytics\\fcast\\forecaster_255439_mp-01-naamsestraat-35-maxim.pkl", "C:\\Users\\uygar\\Desktop\\DataAnalytics\\fcast\\forecaster_255440_mp-02-naamsestraat-57-xior.pkl", "C:\\Users\\uygar\\Desktop\\DataAnalytics\\fcast\\forecaster_255441_mp-03-naamsestraat-62-taste.pkl",
               "C:\\Users\\uygar\\Desktop\\DataAnalytics\\fcast\\forecaster_255442_mp-05-calvariekapel-ku-leuven.pkl", "C:\\Users\\uygar\\Desktop\\DataAnalytics\\fcast\\forecaster_255443_mp-06-parkstraat-2-la-filosovia.pkl", "C:\\Users\\uygar\\Desktop\\DataAnalytics\\fcast\\forecaster_255444_mp-07-naamsestraat-81.pkl"]
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            html.Label('Hour (24-hour format)'),
            dcc.Input(id='hour-input', type='number', min=0, max=23, value=0)
        ], width=3),
        dbc.Col([
        ...
        dbc.Col([
            html.Label('Rainfall'),
            dcc.Slider(id='rainfall-slider', min=0, max=500, step=5, value=10)
        ], width=3),
        dbc.Col([
...
def update_map(date, hour, temperature, rainfall, windspeed):
    # Convert date string to datetime object and add hour and minute
    date = pd.to_datetime(date).replace(hour=hour)
    ...
    exog = pd.DataFrame(index=pd.date_range(start=start_date, freq='H', periods= 1))
    ...
    for forecaster_pkl in forecasters:
        ...
        exog = pd.DataFrame(
            data={
                ...
                'missing': [int(np.isnan(temperature) or np.isnan(rainfall) or np.isnan(windspeed))]
 #TODO HOW?
            }
).set_index('timestamp')
